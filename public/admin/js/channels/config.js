    ...
    channels.listView()
        ...
        .fields([
            ...
            nga.field('icon_url', 'file')
                ...
                .label('Card'),
            nga.field('background', 'file')
                .template('<img src="{{ entry.values.background }}" height="35" width="35" />')
                .cssClasses('hidden-xs')
                .label('Background'),
    ...
    channels.creationView()
        ...
        .fields([
            ...
            nga.field('channel_number', 'number')
                .attributes({ placeholder: 'Must be number' })
                .validation({ required: true })
                .label('Number'),
            nga.field('genre_id', 'reference')
                .targetEntity(admin.getEntity('Genres'))
            ...
            nga.field('icon_url', 'file')
                ...
                .label('Card *'),
            nga.field('background', 'file')
                .uploadInformation({ 'url': '/file-upload/single-file/channels/background', 'apifilename': 'result'})
                .template('<div class="row">'+
                    '<div class="col-xs-12 col-sm-1"><img src="{{ entry.values.background }}" height="40" width="40" /></div>'+
                    '<div class="col-xs-12 col-sm-8"><ma-file-field field="field" value="entry.values.background"></ma-file-field></div>'+
                    '</div>'+
                    '<div class="row"><small id="emailHelp" class="form-text text-muted">préférable de 1920x1200 px</small></div>')
                .validation({
                    validator: function(value) {
                        if (value == null) {
